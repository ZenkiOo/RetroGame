...
testCtrl.gameState.allPositions.push(
  posBowman,
  posSwordsman,
  posVampire,
  posDaemon,
...
test('Метод getUserCharPositions', () => {
...
test('Метод getUserStartPositions', () => {
  const arrTest = [
...
test('onCellEnter показывает ячейку и курсор = pointer', () => {
...
test('Метод getRange должен вернуть корректное значение', () => {
  const bowman = new Bowman(1);
  const bowDist = bowman.distance;
  const bowAttackRange = bowman.attackRange;
  const received = [16, 24, 40, 48, 17, 25, 33, 41, 49, 18, 26, 34, 42, 50];
  expect(testCtrl.board.getRange(32, bowDist)).toEqual(received);
  expect(testCtrl.board.getRange(32, bowAttackRange)).toEqual(received);
});
